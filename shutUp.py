...
async def on_ready():
    ...
    await bot.change_presence(activity=discord.Activity(type=discord.ActivityType.watching, name="Blake without his consent"))
...
#             await bot.get_channel(message.channel.id).send("<@!194192754396364801> " + list[roastNum])
#     #This lets the bot process commands and also listen for the keywords and is NECESSARY or commands wont work
#     await bot.process_commands(message)
...
async def on_message(message):
    ...
    #         await bot.get_channel(message.channel.id).send("<@!194192754396364801> " + list[roastNum])
    # else:
    checkList = ['ur mom', 'youre mom', "you're mom", 'ur mum', 'youre mum', "you're mum"]

    print("Checking for your mom")

    for phrase in checkList:
        if phrase in messageContent:
            await bot.get_channel(message.channel.id).send("good one <:michaelpog:745474772535869581> <:RickyPog:780730650986020894>")
    #This lets the bot process commands and also listen for the keywords and is NECESSARY or commands wont work
    await bot.process_commands(message)
